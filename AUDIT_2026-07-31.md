...
> **Stato correzioni.** Sono stati corretti e verificati: la **CRT** (20 celle), la
> **tabella meteo** (6 righe), i **quattro interventi UX prioritari** e — nel giro
> successivo — **costi di movimento**, **logica EZOC e marker No EZOC**, **Airdrop
> e ritorni Tanks/Jets**, **condizioni di vittoria** (France 1940/41 e scenari
> fan-made) e **air support, bomber in difesa, ritirata nei forti, capacità LSS**.
> Le voci restano qui sotto come documentazione di cosa non andava; quelle ancora
> aperte sono elencate al §6. Suite di test passata da 357 a **444** casi.
...
- ~~Costo 2 MP città/forte nemico, ferrovia negata in attacco, strait +2 MP~~ ✅ **fatto**
- ~~Logica EZOC (stessa unità vs diverse) e marker No EZOC~~ ✅ **fatto**
- ~~Airdrop secondo regola, ritorni Tanks/Jets a 1 turno~~ ✅ **fatto**
- ~~Vittoria France 1940/41 con Belgio+Olanda; condizioni per Barbarossa/Russia~~ ✅ **fatto**
- ~~Nazionalità Air Support, bomber −2 in difesa, retreat in hex con forte, LSS cap 2~~ ✅ **fatto**

Ancora aperto, in ordine di impatto:

1. **Interventi UX successivi**: undo (lo store ha già `combatRollbackState` da generalizzare), tooltip sul canvas, fasi mute (`sequenceStepDescription` è già pronto nell'engine), legenda dei simboli, box economico Asse invisibile e paesi errati in France 1940, identificatori interni mostrati in intestazione.
2. **Regole di dettaglio residue**: Step 2 del combattimento senza scelta segreta simultanea; retreat "one hex gap" semplificato; Isolated senza l'eccezione del bordo mappa; hexside canale non modellato; Strategic Movement senza i divieti 4.1.2 e con ritorno a 1d6 invece che ogni turno; stacking aereo (1 fighter + 1 bomber) non applicato; scelta di avanzata su Assalto in hex svuotato (TODO nel codice); EZOC dell'invasore e attacchi combinati nell'invasione anfibia.
3. **Scenari ufficiali mancanti**: Poland 1939, Scandinavia 1940, France-Italy 1944 (dichiarati "planned": selezionarli carica il setup di France 1940).
4. **Pulizia**: ~590 righe di componenti mai renderizzati (`ActionBanner`, `PhaseToolbar`), dead code in `rules.ts`/`config.ts`/`types.ts`, README che descrive un gioco diverso da quello attuale, build Electron che richiede `rsync`.
5. **Accessibilità**: board non navigabile da tastiera, niente `:focus-visible`, font sotto i 10px.
